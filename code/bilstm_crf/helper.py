...
def extractEntity(sentence, labels):
    ...
    从labels里搜索BM*E, 如果找到，将其提取出来，返回list of entity

    实现:
    - re search BM*E
    - 找到，提取出来; 提取出来的部分用OO覆盖; 继续，直到提取完毕
    """
    entitys = []
...
def buildMap(train_path="train.in"):
    df_train = pd.read_csv(train_path, delimiter='\t', quoting=csv.QUOTE_NONE,
                           skip_blank_lines=False, header=None, names=["char", "label"])
...
def getTest(test_path="test.in", is_validation=False, seq_max_len=200):
    ...
    df_test = pd.read_csv(test_path, delimiter='\t', quoting=csv.QUOTE_NONE,
                          skip_blank_lines=False, header=None, names=["char", "label"])
