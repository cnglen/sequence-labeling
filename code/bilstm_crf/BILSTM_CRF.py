...
class BILSTM_CRF(object):
    ...
    def __init__(self, num_chars, num_classes, num_steps=200, num_epochs=100, embedding_matrix=None, is_training=True, is_crf=True, weight=False):
        ...
        if not is_crf:
            ...
            begin_vec = tf.Variable(np.array([[dummy_val] * self.num_classes + [0]
                                              for _ in range(self.batch_size)]), trainable=False, dtype=tf.float32)
            end_vec = tf.Variable(
                np.array([[0] + [dummy_val] * self.num_classes for _ in range(self.batch_size)]), trainable=False, dtype=tf.float32)
            ...
            # point score
            self.point_score = tf.gather(tf.reshape(self.tags_scores, [-1]), tf.range(
                0, self.batch_size * self.num_steps) * self.num_classes + tf.reshape(self.targets, [self.batch_size * self.num_steps]))
        ...
        for epoch in range(self.num_epochs):
            ...
            for iteration in range(num_iterations):
                ...
                X_train_batch, y_train_batch = helper.nextBatch(
                    X_train, y_train, start_index=iteration * self.batch_size, batch_size=self.batch_size)
                y_train_weight_batch = 1 + \
                    np.array((y_train_batch == label2id['B']) | (y_train_batch == label2id['E']), float)
                ...
                if iteration % 10 == 0:
                    ...
                    precision_train, recall_train, f1_train = self.evaluate(
                        X_train_batch, y_train_batch, predicts_train, id2char, id2label)
                ...
                if iteration % 100 == 0:
                    ...
                    y_val_weight_batch = 1 + \
                        np.array((y_val_batch == label2id['B']) | (y_val_batch == label2id['E']), float)
                    ...
                    precision_val, recall_val, f1_val = self.evaluate(
                        X_val_batch, y_val_batch, predicts_val, id2char, id2label)
    ...
    def predictBatch(self, sess, X, X_str, id2label):
        results = []
        length, max_scores, max_scores_pre = sess.run(
            [self.length, self.max_scores, self.max_scores_pre], feed_dict={self.inputs: X})
    ...
    def evaluate(self, X, y_true, y_pred, id2char, id2label):
        """
        X: 每行是一个样本Feature, 输入字符
        y_true: 每行是一个样本对应的标记
        y_pred: 预测的样本标记

        return:
          precison: 字符级别的, 如果有部分字符识别出来，算部分预测准确
          recall: 同上
        ...
        pred_num = 0
        true_num = 0
        print("X={}, y_true={}, y_pred={}".format(X, y_true, y_pred))
        for i in range(len(y_true)):
            ...
            # print("x={x}, y={y}, y_hat={y_hat}".format(y=y, x=x, y_hat=y_hat))
            true_labels = helper.extractEntity(x, y)
            pred_labels = helper.extractEntity(x, y_hat)
            print("true_labels={}, pred_labels={}".format(true_labels, pred_labels))
            hit_num += len(set(true_labels) & set(pred_labels))  # |true_labes intersection pred_labels|
