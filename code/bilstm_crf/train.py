...
import logging
logging.basicConfig(filename='bilstm_crf.log',
                    level=logging.DEBUG,
...
if __name__ == '__main__':
    ...
    logging.info("preparing train and validation data")
    ...
    with tf.Session(config=config) as sess:
        with tf.device(gpu_config):
            ...
            logging.info("final best f1 is: %f" % (model.max_f1))

    end_time = time.time()
    logging.info("time used %f(hour)" % ((end_time - start_time) / 3600))
